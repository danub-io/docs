...
This document defines the tribarato ecosystem conventions so that AI tools generate code consistent with the rest of the project.
...
- New architecture document → add a `Contem:` inline in SKILL.md (`~/.config/kilo/skills/contexto-tribarato/SKILL.md`) right after the `Leia` line(s) of the corresponding section. Format: 1 line, max 80 chars, comma-separated. Ex: `Contem: OpenTelemetry, tracing, metrics, alertas`

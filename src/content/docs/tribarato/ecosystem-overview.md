...
description: "Complete tribarato data flow: from backend ingestion to frontend display"
---

This document describes the complete flow of a product through the tribarato ecosystem, from raw input in the backend (tribarato_be) to display on the public page (tribarato_fe).
...
|--------|----------|-------------|-------------|
| `Produtos` | Central product catalog | tribarato_be (M1-M7) | tribarato_fe |
| `Reviews` | Press (critic) and user reviews | tribarato_be (M3) + tribarato_fe | Both |
| `Afiliados` | Store links with prices | tribarato_be (M5-M6) | tribarato_fe |
| `historico_precos` | Price history (90 days) | tribarato_be (M5) | — |
| `fila_processamento` | Async jobs (M1-M6) | tribarato_be | — |
| `Guias` / `Guia_Produtos` | Editorial recommendation guides | tribarato_be | tribarato_fe |
| `config_ai_models` | Configured AI models | tribarato_be (M8) | tribarato_be |
| `config_scraping_services` | Scraping services | tribarato_be (M8) | tribarato_be |
| `logs_entrada` | Audit (tokens, cost) | tribarato_be | tribarato_be |
| `conflitos_entrada` | Detected duplicates | tribarato_be (M1) | tribarato_be |
...
Each domain in tribarato_fe has a service that encapsulates SQL queries.

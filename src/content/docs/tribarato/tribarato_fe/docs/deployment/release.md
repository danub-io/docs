...
3. GitHub Actions automatically runs:
   ...
   - **Changelog:** the `scripts/update-changelog.sh` script (in the tribarato_fe repo) generates the entry from git log and commits directly to the `danub-io/docs` repository
...
**Location:** `scripts/update-changelog.sh` in the `danub-io/tribarato_fe` repository
...
The changelog lives in `src/content/docs/tribarato/tribarato_fe/CHANGELOG.md` in the `danub-io/docs` repository.

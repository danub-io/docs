---
title: "Local Setup — tribarato Frontend"
---
...
```bash
turso db create tribarato-dev
turso db show tribarato-dev        # URL
turso db tokens create tribarato-dev  # Token
```
...
Check whether the backend (tribarato_be) has already populated the database with products having `status = 'AprovadoM4'`.
